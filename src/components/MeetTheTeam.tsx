  ...
  const team = [
    {
      ...
      bio: "An OB/GYN with over 14 years of clinical maternal care experience. Dr. Jenkins leads the medical validation of our telemetry alerts, focusing on early preeclampsia detection and remote patient monitoring pathways.",
      specialty: "Maternal-Fetal Telemetry",
      image: "/src/assets/images/dr_sarah_jenkins_1783952527348.jpg",
      icon: <Stethoscope className="h-4 w-4 text-brand-coral" />
    },
    {
      name: "Dr. Michael Ndlovu, MBChB",
      role: "Clinical Director (Regional Coordination)",
      bio: "Based in Manzini, Eswatini, Dr. Ndlovu oversees our regional pilot clinics and coordinate community health integrations. He is passionate about reducing neonatal mortality rate through digital telehealth access.",
      specialty: "Global Maternal Health & Telehealth",
      image: "/src/assets/images/dr_michael_ndlovu_1783952539509.jpg",
    ...
    <div className="bg-white border border-brand-border rounded-3xl p-6 sm:p-10 shadow-sm relative overflow-hidden space-y-10" id="meet-the-team-section">
      ...
      <div className="text-center md:text-left space-y-3 relative z-10">
        <span className="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-semibold bg-brand-pink/20 text-brand-heading border border-brand-pink/30 uppercase tracking-wider">
          <Baby className="h-3.5 w-3.5 mr-1.5 text-brand-coral" /> Clinical Leadership
        </span>
        <h2 className="text-3xl sm:text-4xl font-serif font-bold text-brand-heading tracking-tight">
          The Medical Minds Behind Vytal Bridge
        </h2>
        <p className="text-brand-light-teal text-sm sm:text-base max-w-2xl">
          Meet our dedicated medical advisors and regional leads engineering secure, responsive care channels for mothers and infants.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch relative z-10">
        ...
        <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-6">
          {team.map((member, idx) => (
            <div 
              key={idx}
              className="bg-brand-dark/30 border border-brand-border/60 hover:border-brand-pink/60 rounded-2xl p-5 flex flex-col justify-between transition-all duration-300 hover:shadow-md group"
              id={`team-member-${idx}`}
            >
              <div className="space-y-4">
                {/* Image & Icon Frame */}
                <div className="relative h-48 w-full rounded-xl overflow-hidden bg-brand-light-pink border border-brand-border">
                  <img 
                    src={member.image} 
                    alt={member.name} 
                  ...
                  <div className="absolute bottom-2 right-2 bg-white/90 backdrop-blur-sm p-2 rounded-lg border border-brand-pink/30 shadow-sm flex items-center gap-1.5">
                    {member.icon}
                    <span className="text-[9px] font-bold text-brand-heading uppercase tracking-wide">{member.specialty}</span>
                  </div>
                </div>

                <div className="space-y-1.5">
                  <h4 className="text-base font-semibold text-brand-heading">{member.name}</h4>
                  <p className="text-xs font-medium text-brand-coral uppercase tracking-wider">{member.role}</p>
                  <p className="text-xs text-brand-text leading-relaxed pt-1.5 border-t border-brand-border/40">
                    {member.bio}
                  </p>
                </div>
              </div>
              ...
              <div className="mt-4 pt-3 border-t border-brand-border/30 flex items-center gap-1.5 text-[10px] text-brand-light-teal font-medium">
                <ShieldCheck className="h-3.5 w-3.5 text-brand-teal" /> Board-Certified Active Advisor
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="lg:col-span-4 bg-brand-light-pink/40 border border-brand-pink/30 rounded-2xl p-6 flex flex-col justify-between relative overflow-hidden shadow-sm">
          ...
          <div className="space-y-4 relative z-10">
            ...
            <div className="relative h-40 w-full rounded-xl overflow-hidden border border-brand-pink/40 bg-white">
              <img 
                src="/src/assets/images/newborn_baby_pink_1783952552875.jpg" 
